...
int						get_info(vector <Tetromino> &arr)
{
...
//	long unsigned int	size2 = other.collision_map.size();
	...
	for (i = 0; i < size1; i++)
	{
		...
		cout << " merged map size sjuan"<< merged_map[i].size() << endl;
		for (/*j = 0*/; j < size2; j++)
		{
...
void Tetris::menu()
{
	...
	cout <<  "enter size of rows of the Tetris board";
	cin >> row_string;
	if (my_isdigit(row_string))
		row_size = my_atoi(row_string);
	cout <<  "enter size of columns of the Tetris board";
	cin >> column_string;
